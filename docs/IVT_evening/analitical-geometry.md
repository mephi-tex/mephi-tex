...
![image1](angem_pictures/angem43.png)
...
- $\delta = \begin{equation*} \begin{vmatrix}   
   ...
   a_{12} & a_{22}
   \end{vmatrix} 
   \end{equation*} \neq 0 \implies$ кривая имеет центр симметрии $(x_{0}, y_{0})$ и называется центрально симметричной
- $a_{11}x^{2} + a_{22}y^{2} + 2a_{12}xy + 2a_{13}x + 2a_{23}y + a_{33}' = 0$ 
...
![image1](angem_pictures/hyperboloid.jpg)
- $\dfrac{x^{2}}{a^{2}} + \dfrac{y^{2}}{b^{2}} - \dfrac{z^{2}}{c^{2}} = 1$
#### Двуполостный Гиперболоид
![image1](angem_pictures/hyperboloid1.jpg)
- $\dfrac{x^{2}}{a^{2}} + \dfrac{y^{2}}{b^{2}} - \dfrac{z^{2}}{c^{2}} = -1$

### Классификация нецентральносимметрических поверхностей второго порядка
#### Эллиптический параболоид
![image1](angem_pictures/elparaboloid.png)
...
- $\determinant{A} = \sum\limits_{\sigma \in S_{n}} \operatorname{sgn}({\sigma})a_{1\sigma(1)}a_{2\sigma(2)}\dots a_{n\sigma(n)}$
...
- $\determinant{A} = \sum\limits_{i} a_{ik} \cdot A_{ik}$
- $a_{ik}$ - элемент $A$
...
- $\determinant{A} = 0$, если в квадратной матрице имеются одинаковые или пропорциональные строки
- Если каждый элемент строки домножить на $\lambda$, то получим $\lambda \determinant$
