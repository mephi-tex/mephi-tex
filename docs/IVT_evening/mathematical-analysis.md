...
- $y = f(x), \ x = f^{-1}(y)$
	- $\lim{\Delta t}{0} \left[ \dfrac{\Delta y}{\Delta x} = \dfrac{1}{\dfrac{\Delta x}{\Delta y}} \right] = \dfrac{1}{(f^{-1}(y))'}  = \dfrac{1}{x'(y)}$ 
...
- Пусть $n$ - четное, $f^{(n + 1)}(\xi) \neq 0$. Разложим по формуле Тейлора $f'(x)$.
	- $f'(x) = \dfrac{f^{(n + 1)}(\xi)}{1!}(x - C)^{(n + 1)}$. 
    - $f''(x) = \dfrac{f^{(n + 1)}(\xi)}{1!}(x - C)^{(n + 1)}$. $(x - C)$ меняет знак при переходе, значит $f''(x)$ меняет знак при переходе через $C$.
